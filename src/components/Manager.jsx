...
const Manager = () => {
    ...
    const getPasswords = async () => {
        try {
            ...
            console.error('getPasswords failed', err)
            toast.error('Unable to load passwords (check backend & VITE_API_URL)')
        }
    }
    useEffect(() => {
        getPasswords()
    ...
    const savePassword = async () => {
        if (form.site.length > 3 && form.username.length > 3 && form.password.length > 3) {
            //If any such id exits in the datatbase delte it
            console.log(passwordArray);
            const uniqueId = form.id || uuidv4();

            try {
                if (form.id) {
                    const delResp = await fetch(`${API_URL}/`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: form.id }) })
            ...
            const newEntry = { ...form, id: uniqueId };
              setPasswordArray([...passwordArray, newEntry]);
                console.log(passwordArray);

                const postResp = await fetch(`${API_URL}/`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(newEntry) })
                if (!postResp.ok) throw new Error(`Post failed ${postResp.status}`)
            } catch (err) {
                console.error('savePassword failed', err)
                toast.error('Failed to save password (network or server error)')
                return
            }
            // localStorage.setItem("passwords", JSON.stringify([...passwordArray, {...form, id: uuidv4()}]))
            // console.log([...passwordArray, form])

            // Refetch passwords from the backend to get the updated list
            // await getPasswords();
